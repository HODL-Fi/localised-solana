...
- **`valuation.rs`** matches a `CollateralAsset` by owner + discriminator + stored mint rather than re-deriving `["collateral", mint]`. Equivalent today because listing is the only creation path; either document that invariant or re-derive with the stored bump (~1.5k CU per slot, affordable).
