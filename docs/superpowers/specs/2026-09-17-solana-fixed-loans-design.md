...
- **The allowed-extension set is fixed when a mint is initialized** — Token-2022 extensions are added before `initialize_mint` — so re-running the allowlist on exit could never catch anything the entry policy passed. Only the *values* above remain an issuer's to move.
